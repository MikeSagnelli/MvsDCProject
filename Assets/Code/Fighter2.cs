...
public class Fighter2 : MonoBehaviour {
	...
	void Start () {
		...
		this.tag = "Fighter2";
		health = GameObject.Find ("Health2").GetComponent<Slider> ();
		healthVal = health.value;
		fighter = GameObject.Find ("Name2").GetComponent<Text> ();
		fighter.text = name;
	...
	void OnTriggerEnter2D (Collider2D c){
		if (c.gameObject.tag == "Damage1" && canControl) {
			isHit = true;
		} else if (c.gameObject.tag == "Damage1" && isHit) {
			isHit = true;
		} else {
			isHit = false;
		}
