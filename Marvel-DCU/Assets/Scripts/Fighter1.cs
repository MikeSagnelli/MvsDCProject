...
public class Fighter1 : MonoBehaviour {
	...
	void Start () {
		...
		this.tag = "Fighter1";
		health = GameObject.Find ("P1Health").GetComponent<Slider> ();
		healthVal = health.value;
		fighter = GameObject.Find ("P1Name").GetComponent<Text> ();
		fighter.text = name;
	...
	void OnTriggerEnter2D (Collider2D c){
		if (c.gameObject.tag == "Fighter2" && canControl) {
			isHit = true;
		} else if (c.gameObject.tag == "Fighter2" && isHit) {
			isHit = true;
		} else {
			isHit = false;
		}
